...
X 假定**大量代码将由 AI 代理生成或重写**，因此语言和工具链必须对 AI 友好、机器可读、语义明确：

- **像 Ada 一样严谨**：鼓励显式类型、前置条件/后置条件、清晰的控制流与错误通路，避免"靠习惯理解"的隐式约定
- **AI 优先的风格约定**：规范、文档和格式化规则优先服务于"让编译器/AI 一次就读对"，而不是炫技式省略
- **人类可以更随意**：当人类直接书写或小范围修改代码时，可以在不影响语义确定性的前提下省略部分冗余信息（如显而易见的注解）
- **预编译补全阶段**：在类型检查与代码生成之前，编译器运行一个**预编译阶段**，自动补齐可推导的类型、契约、效果、文档片段等，使内部表示始终处于"完整而冗余"的规范形态

### 设计准则

1. **对机器友好优先**：代码与规范首先要让编译器与 AI 容易理解，其次才是人类的省略与风格
2. **源代码可以简洁，内部表示必须完整**：任何可以通过静态分析可靠推导出的信息，都由预编译阶段自动补齐
3. **AI 与人类协作对等**：语言设计不区分"给 AI 写"和"给人写"两套语法；同一套语法既要让 AI 易于生成，也要让人类易于阅读和局部编辑

---

## 18. 可读性为第一优先级

X 再次明确：**可读性是 X 语言一切设计决策的第一优先级**，当它与性能、语法简洁性、历史包袱、实现难度等目标发生冲突时，优先保证可读性：

- **规范优先**：语言规范、标准库 API、代码风格指南在冲突时，以"让未来读者一眼看懂"为最终裁决标准
- **实现服从**：编译器实现与后端优化必须在不破坏可读性导向语义的前提下进行；必要时宁可放弃某些微优化
- **向后兼容受限**：若历史设计妨碍可读性提升，允许在经过迁移工具和清晰文档支持后做不兼容修改

**设计准则**：当你在两个方案之间犹豫时，选那个让半年后的自己（或别的读者）读起来更轻松的方案，这就是第 18 条的具体执行方式。
...
| 17 | AI 友好代码 | 默认大量由 AI 生成，预编译自动补全细节 |
| 18 | 可读性优先级 | 所有冲突中可读性拥有最高优先权 |
