...
- **示例**：
  ...
  fun foo() {
    let x = 20  // 函数作用域，遮蔽全局变量
    {
      let x = 30  // 块作用域，遮蔽函数变量
    }
  }
  ```
...
// 私有成员（仅当前类可见）
private let secret = "password"

// 保护成员（当前类和子类可见）
protected let familyName = "Smith"

// 模块私有（仅当前模块可见）
module let internal = 42
```
...
// 调用时根据参数类型选择
let result1 = add(1, 2)        // 调用Int版本
let result2 = add(1.5, 2.5)    // 调用Float版本
let result3 = add("a", "b")    // 调用String版本
```
...
// 使用重载的方法
let util = MathUtil()
let result1 = util.compute(5)    // 调用Int版本：10
let result2 = util.compute(5.0)  // 调用Float版本：15.0
```
...
### 函数管道
```x
let result = users |> filter(.active) |> sortBy(.score) |> take(10)
```
...
async fun loadDashboard(uid: Int): Dashboard needs Http, Database {
  let (user, orders, notices) = wait together {
    fetchUser(uid),
    fetchOrders(uid),
    fetchNotices(uid)
  }?
...
fun processOrder(id: Int): Receipt needs Database {
  ...
  let cart  = fetchCart(user.id)?
  let order = placeOrder(cart)?
  Receipt.from(order)
}
...
| let / let mut | 结构 | 不可变绑定 / 可变绑定 | `let x = 5` 或 `let mut x = 5` |
| var | 结构 | 可变变量（向后兼容） | `var count = 0` |
...
async fun placeOrder(userId: Int, items: [Item]): Order
  ...
  let (user, stockOk) = wait together {
