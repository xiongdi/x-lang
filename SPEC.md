...
X 语言使用英文关键字提高可读性，同时保留必要的符号用于表达常见操作。以下是 ASCII 符号在 X 语言中的用法：

### 标点符号

| 符号 | 用途 | 示例 |
|------|------|------|
| `(` `)` | 函数参数、元组、分组、模式匹配 | `f(a, b)`、`(1, 2)`、`(x + y) * z` |
| `[` `]` | 列表字面量、列表模式、索引访问 | `[1, 2, 3]`、`[x, ...]`、`arr[0]` |
| `{` `}` | 代码块、字典字面量、when 表达式 | `{ x + y }`、`{ a: 1 }`、`when x is { }` |
| `,` | 分隔符（参数、列表元素等） | `f(a, b)`、`[1, 2, 3]` |
| `:` | 类型注解、命名参数、字典键值对 | `x: Integer`、`f(name: "A")`、`{ a: 1 }` |
| `;` | 语句分隔符（可选） | `x = 1; y = 2` |
| `.` | 成员访问、方法调用 | `obj.field`、`list.length()` |
| `..` | 范围表达式、范围模式 | `1..10`、`90..100` |
| `_` | 通配符模式、数字分隔符 | `_`、`1_000_000` |
