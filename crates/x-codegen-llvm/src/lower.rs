...
use crate::{LlvmCodeGenError, write_object_or_ir, LlvmConfig};

pub fn generate_code(program: &Program, config: &LlvmConfig) -> Result<Vec<u8>, LlvmCodeGenError> {
    ...
    let fmt_ptr = unsafe {
        builder.build_gep(
            ...
            &[zero, zero],
            "fmt_ptr",
        ).map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?
    };
    ...
    for decl in &program.declarations {
        if let x_parser::ast::Declaration::Function(f) = decl {
            ...
            for (i, param) in f.parameters.iter().enumerate() {
                ...
                let alloca = builder.build_alloca(i64_type, &param.name)
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?;
    ...
    if let Some(&x_main) = func_map.get("main") {
        let ret = builder.build_call(x_main, &[], "ret")
            .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?;
        let ret_val = ret.try_as_basic_value().unwrap_basic().into_int_value();
        let i64_ret = ret_val;
        let i32_ret = builder.build_int_truncate(i64_ret, i32_type, "ret32")
            .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?;
...
fn lower_statement<'ctx>(
    ...
    match stmt {
        Statement::Variable(var) => {
            let alloca = builder.build_alloca(*i64_type, &var.name)
                .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?;
            ...
            builder.build_store(alloca, init)
                .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?;
        ...
        _ => return Err(LlvmCodeGenError::GenerationError(format!("未实现的语句: {:?}", stmt))),
    }
    Ok(())
}
...
fn lower_expr<'ctx>(
    ...
    match expr {
        ...
        Expression::Literal(Literal::Float(_)) => {
            Err(LlvmCodeGenError::GenerationError("float 字面量 codegen 未实现".to_string()))
        }
        Expression::Literal(Literal::Boolean(b)) => {
            Ok(i64_type.const_int(if *b { 1 } else { 0 }, false))
        }
        Expression::Literal(_) => Err(LlvmCodeGenError::GenerationError("该字面量 codegen 未实现".to_string())),
        Expression::Variable(name) => {
            let ptr = locals.get(name)
                .ok_or_else(|| LlvmCodeGenError::GenerationError(format!("未定义变量: {}", name)))?;
            Ok(builder.build_load(*i64_type, *ptr, name)
                .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?
                .into_int_value())
        }
        Expression::Binary(op, left, right) => {
            ...
            let v = match op {
                BinaryOp::Add => builder.build_int_add(l, r, "add")
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?,
                BinaryOp::Sub => builder.build_int_sub(l, r, "sub")
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?,
                BinaryOp::Mul => builder.build_int_mul(l, r, "mul")
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?,
                BinaryOp::Div => builder.build_int_signed_div(l, r, "div")
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?,
                BinaryOp::Mod => builder.build_int_signed_rem(l, r, "mod")
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?,
                BinaryOp::Less => builder.build_int_compare(inkwell::IntPredicate::SLT, l, r, "lt")
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?,
                BinaryOp::LessEqual => builder.build_int_compare(inkwell::IntPredicate::SLE, l, r, "le")
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?,
                BinaryOp::Greater => builder.build_int_compare(inkwell::IntPredicate::SGT, l, r, "gt")
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?,
                BinaryOp::GreaterEqual => builder.build_int_compare(inkwell::IntPredicate::SGE, l, r, "ge")
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?,
                BinaryOp::Equal => builder.build_int_compare(inkwell::IntPredicate::EQ, l, r, "eq")
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?,
                BinaryOp::NotEqual => builder.build_int_compare(inkwell::IntPredicate::NE, l, r, "ne")
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?,
                _ => return Err(LlvmCodeGenError::GenerationError(format!("未实现二元运算: {:?}", op))),
            };
            let iv = v;
            if iv.get_type() == *i1_type {
                Ok(builder.build_int_z_extend(iv, *i64_type, "zext")
                    .map_err(|e| LlvmCodeGenError::GenerationError(e.to_string()))?)
            } else {
                Ok(iv)
            }
        ...
        Expression::Call(callee, args) => {
            if let Expression::Variable(ref name) = callee.as_ref() {
                ...
                let func = func_map.get(name)
                    .ok_or_else(|| LlvmCodeGenError::GenerationError(format!("未定义函数: {}", name)))?;
            ...
            Err(LlvmCodeGenError::GenerationError("仅支持命名函数调用".to_string()))
        }
        Expression::Parenthesized(inner) => {
            lower_expr(context, module, builder, inner, locals, func_map, fmt_ptr, printf, i64_type, i1_type)
        }
        _ => Err(LlvmCodeGenError::GenerationError(format!("未实现表达式: {:?}", expr))),
    }
}

fn write_object_or_ir(
    module: &Module,
    config: &LlvmConfig,
    ...
    match config.target {
        ...
        crate::LlvmTargetKind::Native => {
            ...
            let target = LlvmTarget::from_triple(&target_triple)
                .map_err(|e| LlvmCodeGenError::TargetMachineError(e.to_string()))?;
            let target_machine = target
                ...
                .ok_or_else(|| LlvmCodeGenError::TargetMachineError("create_target_machine failed".to_string()))?;
            let object_code = target_machine
                .write_to_memory_buffer(module, FileType::Object)
                .map_err(|e: inkwell::support::LLVMString| LlvmCodeGenError::GenerationError(e.to_string()))?;
            Ok(object_code.as_slice().to_vec())
        }
        crate::LlvmTargetKind::Wasm => {
            Err(LlvmCodeGenError::Unsupported("Wasm目标未实现".to_string()))
        }
    }
}
